...
insertUI(){
  ...
  for (( i=1; i<= $noOfCols; i++))
    ...
    if [[ $ifPrimary == 'primary' && $intOrStr == 'number' ]]
      then
      ...
      while [[ $emptyBool == 'false' || $numberBool == 'false' || $nullBool == 'false' || $uniqueBool == 'false' ]]
        do
      ...
      if [ $i == 1 ] # if it is the first column create a new line 
        then
          val+=":"
          echo $val >> $1
        elif [ $i == $noOfCols ]  # if it is the last column don't append : to val
        then
          sed -i '$s/$/'$val'/' $1
        else
          val+=":"
    ...
    if [[ $ifPrimary == 'primary' && $intOrStr == 'string' ]]
      then
      echo `head -2 $1 | tail -1 | cut -d: -f$i`
      read val
      ...
      nullBool=`isNotNull $val`
      uniqueBool=`isUnique $i $val $1`
      while [[ $emptyBool == 'false' || $stringBool == 'false' || $nullBool == 'false' || $uniqueBool == 'false' ]]
        do
          echo "This value is invalid"
    ...
    if [[ $ifPrimary == 'notprimary' && $intOrStr == 'number' ]]
      then
      echo `head -2 $1 | tail -1 | cut -d: -f$i`
      read val
      ...
      emptyBool=`isNotEmpty $val`
      numberBool=`isNumber $val`
      while [[ $emptyBool == 'false' || $numberBool == 'false' ]]
        do
          echo "This value is invalid"
    ...
    if [[ $ifPrimary == 'notprimary' && $intOrStr == 'string' ]]
      then
      echo `head -2 $1 | tail -1 | cut -d: -f$i`
      read val
      ...
      emptyBool=`isNotEmpty $val`
      stringBool=`isString $val`
      while [[ $emptyBool == 'false' || $stringBool == 'false'  ]]
        do
          echo "This value is invalid"
